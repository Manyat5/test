 ...
 * <p>
 * 通过在循环中而不是在方法调用中执行操作，循环是在希望最小化调用测试方法的开销的情况下进行的。
 * 不要相信这种说法;当我们允许优化器合并循环迭代时，您将看到更多神奇的事情发生。
 * <p>
 * 结果：随着循环数的增加，每个循环的时间也随之减少。JIT会对循环进行优化，做循环消除
 * <p>
 * OperationsPerInvocation(x)注解的作用是对当前调用的分数除以x，如该例子中进行了10次循环，则x=10，平均每次循环的时间为最后的分数
 */
@State(Scope.Thread)
